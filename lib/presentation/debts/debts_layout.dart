...
class _DebtsLayoutState extends State<DebtsLayout> {
  ...
  Future<void> _createClient() async => CreateClientDialog.show(context);
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 12),
              TextField(controller: address, keyboardType: TextInputType.streetAddress, maxLines: 2, decoration: const InputDecoration(labelText: 'Dirección')),
            ]),
          ),
          actions: [
  ...
  Future<void> _addPayment(Client client, DebtAccount account) async {
    ...
    final saved = context.read<DebtProvider>().recordPayment(clientId: client.id, clientName: client.name, amount: received);
    if (!saved) {
    ...
    AppAlert.show(
      context,
      change > 0.005 ? 'Se registró un abono de ${_money(applied)}. Cambio: ${_money(change)}.' : 'Se registró un abono de ${_money(applied)}.',
      title: 'Abono registrado',
      type: AppAlertType.success,
    );
  }

  Future<void> _showPurchaseHistory(Client client, List<SaleRecord> sales) => ClientPurchaseHistoryDialog.show(context, clientName: client.name, sales: sales);
        ...
        Padding(
          ...
          child: Column(children: [
            ...
            Expanded(child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Expanded(flex: 3, child: _buildClientsPanel(clients, debtProvider, salesProvider)),
  ...
  Widget _buildSummary(DebtProvider provider) => Row(children: [
  ...
  Widget _buildClientsPanel(List<Client> clients, DebtProvider debtProvider, SalesProvider salesProvider) => _Panel(
    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [const Text('Clientes', style: AppTextStyles.sectionTitle), const Spacer(), Text('${clients.length}', style: const TextStyle(color: AppColors.textMuted, fontSize: 11))]),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: ProductSearchBar(controller: _searchController, hintText: 'Buscar cliente...', onChanged: (value) => setState(() => _searchQuery = value))),
        const SizedBox(width: 10),
        DropdownButtonHideUnderline(child: DropdownButton<_DebtFilter>(
          value: _filter,
  ...
  Widget _buildHistoryPanel(DebtProvider provider) => _Panel(
    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 10),
      Expanded(child: provider.movements.isEmpty
          ? const _EmptyState(icon: Icons.receipt_long_outlined, message: 'Todavía no hay movimientos.')
          : ListView.separated(itemCount: provider.movements.length, separatorBuilder: (_, __) => const SizedBox(height: 7), itemBuilder: (_, index) => _MovementTile(provider.movements[index]))),
    ]),
  );
  ...
  List<Client> _filteredClients(List<Client> clients, DebtProvider provider) {
    ...
    final result = clients.where((client) {
      ...
      final remaining = provider.accountFor(client.id)?.remaining ?? 0;
...
class _ClientDebtCard extends StatelessWidget {
  final Client client; final DebtAccount account; final int purchaseCount; final VoidCallback onEdit; final VoidCallback onHistory; final VoidCallback? onPayment;
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(client.name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)), if (client.phone.isNotEmpty) Text(client.phone, style: const TextStyle(fontSize: 10, color: AppColors.textMuted))])),
          if (purchaseCount > 0) TextButton.icon(onPressed: onHistory, icon: const Icon(Icons.receipt_long_outlined, size: 15), label: Text('$purchaseCount compras'), style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 7), visualDensity: VisualDensity.compact, textStyle: const TextStyle(fontSize: 10))),
        ...
        Row(children: [
          ...
          Expanded(child: _AmountBox(label: 'Abonado', value: _money(account.totalPaid), color: AppColors.successGreen)),
          const SizedBox(width: 7),
        ...
        Row(children: [Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(5), child: LinearProgressIndicator(value: progress, minHeight: 6, backgroundColor: AppColors.border, valueColor: const AlwaysStoppedAnimation<Color>(AppColors.successGreen)))), const SizedBox(width: 8), Text('${(progress * 100).round()}%', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.w600))]),
        const SizedBox(height: 8),
        Align(alignment: Alignment.centerRight, child: FilledButton.tonalIcon(onPressed: onPayment, icon: const Icon(Icons.payments_outlined, size: 16), label: const Text('Abonar'), style: FilledButton.styleFrom(minimumSize: const Size(0, 34), padding: const EdgeInsets.symmetric(horizontal: 12), textStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(movement.clientName, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), const SizedBox(height: 2), Text(label, style: TextStyle(fontSize: 10, color: color, fontWeight: FontWeight.w600)), const SizedBox(height: 2), Text('$date · $time', style: const TextStyle(fontSize: 9, color: AppColors.textMuted)), if (movement.reference != null && movement.reference!.isNotEmpty) Text('Ticket #${movement.reference}', style: const TextStyle(fontSize: 9, color: AppColors.textSecondary))])),
...
class _DialogBalanceRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [Text(label, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)), const Spacer(), Text(value, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.dangerRed))]);
}
...
class _EmptyState extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 30, color: AppColors.textMuted), const SizedBox(height: 8), Text(message, textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, color: AppColors.textMuted))]));
}
...
class _PaymentDialog extends StatefulWidget {
  ...
  static Future<double?> show(BuildContext context, {required String clientName, required double debt}) => showDialog<double>(
    context: context,
...
class _PaymentDialogState extends State<_PaymentDialog> {
  ...
  @override
  void dispose() { _keypadEntry?.remove(); _keypadEntry = null; _controller.dispose(); super.dispose(); }
  ...
  void _showKeypad() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_keypadEntry == null) {
        ...
        _keypadEntry = OverlayEntry(builder: (_) => Positioned(
  ...
  void _updateKeypadPosition() {
    ...
    _keypadPosition = Offset(desiredLeft.clamp(8, maxLeft < 8 ? 8 : maxLeft), desiredTop.clamp(8, maxTop < 8 ? 8 : maxTop));
  }

  void _closeKeypad() { _keypadEntry?.remove(); _keypadEntry = null; }
  void _setText(String value) { _controller.value = TextEditingValue(text: value, selection: TextSelection.collapsed(offset: value.length)); setState(() {}); }
  void _input(String digit) => _setText('${_controller.text}$digit');
  void _decimal() { if (_controller.text.contains('.')) return; _setText(_controller.text.isEmpty ? '0.' : '${_controller.text}.'); }
  void _backspace() { if (_controller.text.isEmpty) return; _setText(_controller.text.substring(0, _controller.text.length - 1)); }
  ...
  Widget build(BuildContext context) => TapRegion(
    ...
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(color: AppColors.successGreen.withAlpha(16), borderRadius: BorderRadius.circular(9)), child: const Icon(Icons.payments_outlined, color: AppColors.successGreen, size: 20)),
          const SizedBox(width: 9),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Abonar a ${widget.clientName}', style: AppTextStyles.sectionTitle), const SizedBox(height: 2), const Text('Registra el efectivo recibido y calcula el cambio.', style: TextStyle(fontSize: 9, color: AppColors.textSecondary))])),
        ...
        Container(
          ...
          child: TextField(
            ...
            readOnly: true,
            onTap: _showKeypad,
            decoration: const InputDecoration(labelText: 'Efectivo recibido', prefixText: '\$ ', prefixIcon: Icon(Icons.payments_outlined, size: 18)),
          ),
        ),
        const SizedBox(height: 8),
        AnimatedSwitcher(
          ...
          child: _change > 0.005
              ? Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.warningOrange.withAlpha(12), borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.warningOrange.withAlpha(35))),
                  child: Row(children: [const Icon(Icons.currency_exchange, size: 16, color: AppColors.warningOrange), const SizedBox(width: 7), const Expanded(child: Text('Cambio a entregar', style: TextStyle(fontSize: 10, color: AppColors.textSecondary))), Text('\$${_change.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.warningOrange))]),
                )
              : const SizedBox(key: ValueKey('no-change'), height: 36),
        ),
        const SizedBox(height: 8),
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Cancelar')),
          const SizedBox(width: 7),
          FilledButton.icon(onPressed: _received > 0 ? () => Navigator.of(context).pop(_received) : null, icon: const Icon(Icons.check_rounded, size: 16), label: const Text('Registrar abono')),
        ]),
      ]),
    ),
  );
