...
class _DebtsLayoutState extends State<DebtsLayout> {
      ...
      await showDialog<void>(context: context, builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () {
            ...
            if (!context.read<CatalogProvider>().updateClient(updated)) { AppAlert.show(context, 'No se pudo actualizar el cliente.', title: 'Error al actualizar', type: AppAlertType.error); return; }
  ...
  Widget _clientsPanel(List<Client> clients, List<ClientGroup> groups, DebtProvider debts, SalesProvider sales, List<Client> allClients) => _Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    ...
    Row(children: [Expanded(child: ProductSearchBar(controller: _searchController, hintText: 'Buscar cliente o grupo...', onChanged: (v) => setState(() => _searchQuery = v))), const SizedBox(width: 10), DropdownButtonHideUnderline(child: DropdownButton<_DebtFilter>(value: _filter, items: const [DropdownMenuItem(value: _DebtFilter.all, child: Text('Todos')), DropdownMenuItem(value: _DebtFilter.pending, child: Text('Con deuda')), DropdownMenuItem(value: _DebtFilter.paid, child: Text('Pagados'))], onChanged: (v) { if (v != null) setState(() => _filter = v); }))]),
    const SizedBox(height: 10),
    Expanded(child: clients.isEmpty && groups.isEmpty ? const _EmptyState('No hay clientes o grupos que coincidan con el filtro.', Icons.people_outline) : ListView.separated(
      padding: const EdgeInsets.only(bottom: 80), itemCount: groups.length + clients.length, separatorBuilder: (_, __) => const SizedBox(height: 8),
  ...
  Widget _groupCard(ClientGroup group, List<Client> allClients, DebtProvider debts, SalesProvider sales) {
    ...
    return Container(decoration: BoxDecoration(color: AppColors.inputBackground, borderRadius: BorderRadius.circular(11), border: Border.all(color: AppColors.border)), child: Column(children: [
      InkWell(onTap: () => setState(() => expanded ? _expandedGroups.remove(group.id) : _expandedGroups.add(group.id)), borderRadius: BorderRadius.circular(11), child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
        Row(children: [Container(width: 36, height: 36, decoration: BoxDecoration(color: AppColors.primary.withAlpha(18), shape: BoxShape.circle), child: const Icon(Icons.groups_outlined, color: AppColors.primary, size: 20)), const SizedBox(width: 9), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(group.name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textPrimary)), Text('${members.length} integrantes', style: const TextStyle(fontSize: 10, color: AppColors.textMuted))])), Text(_money(remaining), style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: remaining > .005 ? AppColors.dangerRed : AppColors.successGreen)), const SizedBox(width: 5), Icon(expanded ? Icons.expand_less : Icons.expand_more, color: AppColors.textSecondary)]),
        const SizedBox(height: 9), Row(children: [Expanded(child: _AmountBox('Deuda', _money(debt), AppColors.dangerRed)), const SizedBox(width: 7), Expanded(child: _AmountBox('Abonado', _money(paid), AppColors.successGreen)), const SizedBox(width: 7), Expanded(child: _AmountBox('Restante', _money(remaining), remaining > .005 ? AppColors.warningOrange : AppColors.successGreen))]),
      ]))),
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.all(9), decoration: BoxDecoration(color: AppColors.inputBackground, borderRadius: BorderRadius.circular(9), border: Border.all(color: AppColors.border)), child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(movement.clientName, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        Text(isPayment ? 'Abono' : 'Fiado', style: const TextStyle(fontSize: 9, color: AppColors.textMuted)),
      ])),
...
class _EmptyState extends StatelessWidget { final String message; final IconData icon; const _EmptyState(this.message, this.icon); @override Widget build(BuildContext context) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 30, color: AppColors.textMuted), const SizedBox(height: 8), Text(message, textAlign: TextAlign.center, style: const TextStyle(color: AppColors.textMuted, fontSize: 11))])); }
...
class _PaymentDialogState extends State<_PaymentDialog> {
  final _controller = TextEditingController(); final _focusNode = FocusNode();
