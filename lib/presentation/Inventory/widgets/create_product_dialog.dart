...
class _CreateProductDialogState extends State<CreateProductDialog> {
  ...
  bool get _isEditing =>
      widget.product != null &&
      (widget.product?['id']?.toString().trim().isNotEmpty ?? false);
  ...
  void initState() {
    ...
    _selectedBrand = _nullable(p?['brand']);
    _selectedDistributor = _nullable(p?['department']);
    _imageData = _nullable(p?['imageData']);
    _hasGroupPricing =
        p?['hasGroupPricing'] == true ||
        p?['hasGroupPricing']?.toString().toLowerCase() == 'true';
    _groupExpanded = _hasGroupPricing;
    _groupQuantityController.text = _hasGroupPricing
        ? _readInt(p?['groupQuantity']).toString()
        : '';
  ...
  Future<void> _createDistributor() async =>
      ManageDistributorsDialog.show(context);
  ...
  int _currentValue(TextEditingController c, int fallback) =>
  ...
  Future<void> _save() async {
    if (!_validate()) return;
    final physicalStock = _readInt(_stockController.text);
    final product = Product(
      ...
      hasGroupPricing: _hasGroupPricing,
      groupQuantity: _hasGroupPricing
    ...
    if (_isEditing) {
      ...
      final updated = provider.updateProduct(
        product.copyWith(stock: _registeredStock),
      );
      if (!updated) {
        AppAlert.show(
          context,
      ...
      if (!provider.updateStockToPhysical(product.id, physicalStock)) {
        AppAlert.show(
          context,
          'No se pudo actualizar el inventario físico.',
          title: 'No se pudo guardar',
          type: AppAlertType.warning,
        );
        return;
      ...
      if (!added) {
        AppAlert.show(
          context,
          'El producto ya existe o coincide con otro producto registrado.',
          title: 'No se pudo guardar',
          type: AppAlertType.warning,
        );
        return;
  ...
  Widget build(BuildContext context) {
    final adjustment = _stock - _registeredStock;
    return Center(
        ...
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                Text(
                  'Vender este producto por grupos',
                  style: TextStyle(
                    fontSize: 12,
                ...
                Row(
                  children: [
                    ...
                    Switch.adaptive(
                      ...
                      onChanged: (value) =>
                          setState(() => _hasGroupPricing = value),
                    ),
                  ],
                ),
                  ...
                  children: [
                    Expanded(
                      child: _field(
                        _groupQuantityController,
                        'Unidades por grupo',
                        invalid: _invalid('groupQuantity'),
                        changed: (_) => _clearError('groupQuantity'),
                    ...
                    Expanded(
                      child: _field(
                        ...
                        'Precio por grupo',
                        prefix: '\$ ',
                        invalid: _invalid('groupPrice'),
                        changed: (_) => _clearError('groupPrice'),
                        type: const TextInputType.numberWithOptions(
                          decimal: true,
                        ),
                        formatter: const DecimalInputFormatter(
  ...
  Widget _stockSummary(int registered, int physical, int adjustment) =>
      Container(
        width: double.infinity,
