...
class _PurchasesLayoutState extends State<PurchasesLayout> {
  ...
  Widget build(BuildContext context) {
    ...
    final metrics = <PeriodSummaryMetric>[
      PeriodSummaryMetric('Compras', _money(purchaseTotal)), PeriodSummaryMetric('Ventas', _money(salesTotal)),
      PeriodSummaryMetric('Ganancia bruta', _money(profit), valueColor: AppColors.successGreen), PeriodSummaryMetric('Compras registradas', '${purchases.length}'),
      PeriodSummaryMetric('Unidades recibidas', '$itemCount'), PeriodSummaryMetric('Bonificaciones', '$bonusCount'), PeriodSummaryMetric('Distribuidoras', '$supplierCount'),
    ];
    ...
    return Padding(
      ...
      child: Stack(children: [
        ...
        Positioned(right: 0, bottom: 0, child: FloatingActionButton(onPressed: _newPurchase, tooltip: 'Nueva compra', backgroundColor: AppColors.primary, shape: const CircleBorder(), child: const Icon(Icons.add_shopping_cart_outlined, color: Colors.white))),
      ]),
    );
  }
  ...
  Future<void> _newPurchase() async { final created = await PurchaseCreationDialog.show(context); if (created == true && mounted) setState(() {}); }

  Future<void> _pickDate() async { if (_period == _PurchasePeriod.custom) return _pickRange(); final picked = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2100), initialDate: _anchorDate); if (picked != null && mounted) setState(() => _anchorDate = picked); }
  Future<void> _pickRange() async { final start = _customStart ?? _anchorDate; final end = _customEnd ?? start; final picked = await showDateRangePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2100), initialDateRange: DateTimeRange(start: start, end: end.isBefore(start) ? start : end)); if (picked == null || !mounted) return; setState(() { _period = _PurchasePeriod.custom; _customStart = picked.start; _customEnd = picked.end; _anchorDate = picked.start; }); }
...
class _PurchaseDetailDialog extends StatelessWidget {
  ...
  Future<void> _modify(BuildContext context) async {
    final updated = await PurchaseCreationDialog.show(context, purchase: purchase);
    if (updated == true && context.mounted) Navigator.pop(context, true);
  }

  Future<void> _delete(BuildContext context) async {
    final confirmed = await showDialog<bool>(context: context, barrierColor: AppColors.overlayBackground, builder: (_) => const _PurchaseDeleteConfirmationDialog());
  ...
  Widget build(BuildContext context) => Dialog(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 760, maxHeight: 700), child: Column(children: [
    ...
    Padding(padding: const EdgeInsets.all(16), child: Row(children: [Expanded(child: _info('Factura', purchase.invoiceNumber.isEmpty ? '—' : purchase.invoiceNumber)), Expanded(child: _info('Fecha', _date(purchase.arrivalAt))), Expanded(child: _info('Hora', _time(purchase.arrivalAt))), Expanded(child: _info('Pago', purchase.paymentMethod.isEmpty ? 'Contado' : purchase.paymentMethod)), Expanded(child: _info('Total', _money(purchase.total), strong: true))])),
    const Divider(height: 1),
    Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 10), child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [OutlinedButton.icon(onPressed: () => _modify(context), icon: const Icon(Icons.edit_outlined, size: 17), label: const Text('Modificar compra')), const SizedBox(width: 8), OutlinedButton.icon(onPressed: () => _delete(context), icon: const Icon(Icons.delete_outline, size: 17), label: const Text('Eliminar compra'), style: OutlinedButton.styleFrom(foregroundColor: AppColors.dangerRed))])),
    const Divider(height: 1),
    Expanded(child: ListView.separated(padding: const EdgeInsets.all(16), itemCount: purchase.items.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, index) => _item(purchase.items[index]))),
  ]));

  Widget _info(String label, String value, {bool strong = false}) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 9, color: AppColors.textMuted)), const SizedBox(height: 3), Text(value, style: TextStyle(fontSize: 12, fontWeight: strong ? FontWeight.w900 : FontWeight.w700, color: strong ? AppColors.primary : null))]);

  Widget _item(PurchaseItemRecord item) {
    Uint8List? bytes;
    if (item.imageData.trim().isNotEmpty) { try { bytes = base64Decode(item.imageData.contains(',') ? item.imageData.split(',').last : item.imageData); } catch (_) {} }
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), decoration: BoxDecoration(color: AppColors.inputBackground, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.border)), child: Row(children: [SizedBox(width: 42, height: 42, child: bytes == null ? const Icon(Icons.image_outlined, color: AppColors.textMuted) : Image.memory(bytes, fit: BoxFit.contain)), const SizedBox(width: 9), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.productName, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), const SizedBox(height: 3), Text('${item.totalQuantity} recibidas · ${item.bonusQuantity} bonificadas', style: const TextStyle(fontSize: 9, color: AppColors.textSecondary))])), Text(_money(item.total), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700))]));
  }
}
...
class _PurchaseDeleteConfirmationDialog extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Dialog(backgroundColor: Colors.transparent, elevation: 0, child: Container(width: 430, padding: const EdgeInsets.fromLTRB(24, 24, 24, 20), decoration: BoxDecoration(color: AppColors.cardBackground, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.border), boxShadow: const [BoxShadow(color: AppColors.shadowColor, blurRadius: 24, offset: Offset(0, 10))]), child: Column(mainAxisSize: MainAxisSize.min, children: [
    Container(width: 62, height: 62, decoration: BoxDecoration(color: AppColors.dangerRed.withAlpha(20), shape: BoxShape.circle), child: const Icon(Icons.delete_forever_rounded, color: AppColors.dangerRed, size: 31)),
    const SizedBox(height: 15), const Text('Eliminar compra', textAlign: TextAlign.center, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: AppColors.textPrimary)),
    const SizedBox(height: 8), const Text('Esta compra se eliminará del historial y se revertirán las unidades que agregó al inventario.', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, height: 1.45, color: AppColors.textSecondary)),
    const SizedBox(height: 14), Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: AppColors.dangerRed.withAlpha(10), borderRadius: BorderRadius.circular(11), border: Border.all(color: AppColors.dangerRed.withAlpha(45))), child: const Row(children: [Icon(Icons.warning_amber_rounded, color: AppColors.dangerRed, size: 19), SizedBox(width: 8), Expanded(child: Text('Esta acción no se puede deshacer.', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.dangerRed)))])),
    const SizedBox(height: 20), Row(children: [Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(context, false), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(44), foregroundColor: AppColors.textSecondary, side: const BorderSide(color: AppColors.border), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(11))), child: const Text('Cancelar', style: TextStyle(fontWeight: FontWeight.w700)))), const SizedBox(width: 10), Expanded(child: FilledButton.icon(onPressed: () => Navigator.pop(context, true), style: FilledButton.styleFrom(backgroundColor: AppColors.dangerRed, foregroundColor: Colors.white, minimumSize: const Size.fromHeight(44), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(11))), icon: const Icon(Icons.delete_outline_rounded, size: 18), label: const Text('Eliminar compra', style: TextStyle(fontWeight: FontWeight.w800))))]),
  ]));
}
