...
class _PurchasesLayoutState extends State<PurchasesLayout> {
  ...
  List<PurchaseRecord> _filterPurchases(List<PurchaseRecord> purchases) {
    final range = _range();
  ...
  Widget build(BuildContext context) {
    ...
    final range = _range();
    final purchaseTotal = purchases.fold<double>(0, (sum, purchase) => sum + purchase.total);
    final salesTotal = sales.fold<double>(0, (sum, sale) => sum + sale.total);
    final grossProfit = sales.fold<double>(0, (sum, sale) => sum + sale.items.fold<double>(0, (inner, item) => inner + ((item.unitPrice - item.cost) * item.quantity) - item.discount));
    final itemCount = purchases.fold<int>(0, (sum, purchase) => sum + purchase.itemCount);
    final supplierCount = purchases.map((purchase) => purchase.distributorName.toLowerCase()).toSet().length;
    ...
    final summaryMetrics = <PeriodSummaryMetric>[
      ...
      PeriodSummaryMetric('Ganancia bruta', _money(grossProfit), valueColor: AppColors.successGreen),
    ...
    return Padding(
      ...
      child: Stack(children: [
        ...
        Positioned(right: 0, bottom: 0, child: FloatingActionButton(shape: const CircleBorder(), backgroundColor: AppColors.primary, tooltip: 'Nueva compra', onPressed: _newPurchase, child: const Icon(Icons.add_shopping_cart_outlined, color: Colors.white))),
      ]),
    );
  }
  ...
  Widget _buildHeader() => Row(children: [
    ...
    OutlinedButton.icon(onPressed: _pickDate, icon: const Icon(Icons.calendar_today_outlined, size: 16), label: Text(_period == _PurchasePeriod.custom ? 'Elegir rango' : _dateLabel())),
  ...
  Widget _buildMetrics(double purchases, double sales, double profit) => Row(children: [
    Expanded(child: _metric('Compras', purchases, Icons.shopping_bag_outlined, AppColors.dangerRed)), const SizedBox(width: 8),
    Expanded(child: _metric('Ventas', sales, Icons.point_of_sale_outlined, AppColors.primary)), const SizedBox(width: 8),
    Expanded(child: _metric('Ganancia bruta', profit, Icons.trending_up_outlined, AppColors.successGreen)),
  ]);

  Widget _metric(String label, double amount, IconData icon, Color color) => Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: AppColors.cardBackground, borderRadius: BorderRadius.circular(AppDimensions.cardRadius), border: Border.all(color: AppColors.border), boxShadow: const [BoxShadow(color: AppColors.shadowColor, blurRadius: 8, offset: Offset(0, 3))]), child: Row(children: [Icon(icon, size: 20, color: color), const SizedBox(width: 9), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary)), const SizedBox(height: 2), Text(_money(amount), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700))]))]));

  Widget _buildFilters() => Row(children: [
    Expanded(child: ProductSearchBar(controller: _searchController, hintText: 'Buscar proveedor o factura...', onChanged: (_) => setState(() {}))), const SizedBox(width: 8),
    PopupMenuButton<_PurchasePeriod>(onSelected: (value) => value == _PurchasePeriod.custom ? _pickRange() : setState(() => _period = value), itemBuilder: (_) => const [PopupMenuItem(value: _PurchasePeriod.daily, child: Text('Diario')), PopupMenuItem(value: _PurchasePeriod.weekly, child: Text('Semanal')), PopupMenuItem(value: _PurchasePeriod.monthly, child: Text('Mensual')), PopupMenuItem(value: _PurchasePeriod.yearly, child: Text('Anual')), PopupMenuItem(value: _PurchasePeriod.custom, child: Text('Rango personalizado'))], child: _menuSurface(Icons.tune_outlined, _periodLabel())),
  ]);
  ...
  Widget _buildList(List<PurchaseRecord> purchases) => HistoryTablePanel(
    title: 'Historial de compras', icon: Icons.receipt_long_outlined, itemCount: purchases.length, header: _buildListHeader(),
  ...
  Widget _buildListHeader() => Container(padding: const EdgeInsets.fromLTRB(14, 10, 14, 9), decoration: const BoxDecoration(color: AppColors.inputBackground, border: Border(bottom: BorderSide(color: AppColors.border))), child: const Row(children: [Expanded(flex: 2, child: Text('Proveedor / factura', style: AppTextStyles.ticketLabel)), SizedBox(width: 105, child: Text('Fecha / hora', style: AppTextStyles.ticketLabel)), Expanded(child: Text('Artículos', style: AppTextStyles.ticketLabel)), SizedBox(width: 90, child: Text('Pago', style: AppTextStyles.ticketLabel)), SizedBox(width: 95, child: Text('Total', textAlign: TextAlign.right, style: AppTextStyles.ticketLabel)), SizedBox(width: 26)]));
  ...
  Widget _purchaseRow(PurchaseRecord purchase) {
    ...
    return InkWell(onTap: () => _showDetail(purchase), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), child: Row(children: [
      Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(purchase.distributorName, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)), const SizedBox(height: 2), Text(purchase.invoiceNumber.isEmpty ? 'Sin número de factura' : 'Factura ${purchase.invoiceNumber}', style: const TextStyle(fontSize: 9, color: AppColors.textMuted))])),
      ...
      Expanded(child: Text('${purchase.itemCount} artículo${purchase.itemCount == 1 ? '' : 's'}', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary))),
      SizedBox(width: 90, child: _paymentBadge(purchase.paymentMethod)),
  ...
  void _newPurchase() {
    showDialog<void>(context: context, builder: (_) => const AlertDialog(title: Text('Nueva compra'), content: Text('Aquí conectaremos el formulario de ingreso cuando definamos el segundo boceto.'), actions: []));
  }

  void _showDetail(PurchaseRecord purchase) {
    showDialog<void>(context: context, builder: (_) => _PurchaseDetailDialog(purchase));
  }
}
...
class _PurchaseDetailDialog extends StatelessWidget {
    ...
    return Dialog(
      child: ConstrainedBox(
        ...
        child: Column(children: [
          ...
          Padding(padding: const EdgeInsets.all(16), child: Row(children: [Expanded(child: _info('Factura', purchase.invoiceNumber.isEmpty ? '—' : purchase.invoiceNumber)), Expanded(child: _info('Fecha de llegada', _date(purchase.arrivalAt))), Expanded(child: _info('Forma de pago', purchase.paymentMethod))])),
          const Divider(height: 1),
          Expanded(child: ListView.separated(padding: const EdgeInsets.all(16), itemCount: purchase.items.length, separatorBuilder: (_, __) => const SizedBox(height: 7), itemBuilder: (_, index) => _purchaseItem(purchase.items[index]))),
          Padding(padding: const EdgeInsets.all(16), child: Row(children: [const Spacer(), const Text('Total', style: TextStyle(fontWeight: FontWeight.w700)), const SizedBox(width: 28), Text('\$${purchase.total.toStringAsFixed(2)}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.primary))])),
        ]),
      ),
    );
  }
  ...
  Widget _purchaseItem(PurchaseItemRecord item) {
    final bytes = item.imageData.trim().isEmpty ? null : _decode(item.imageData);
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), decoration: BoxDecoration(color: AppColors.inputBackground, borderRadius: BorderRadius.circular(9), border: Border.all(color: AppColors.border)), child: Row(children: [
      Container(width: 48, height: 48, clipBehavior: Clip.antiAlias, decoration: BoxDecoration(color: AppColors.cardBackground, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.border)), child: bytes == null ? const Icon(Icons.image_outlined, color: AppColors.textMuted) : Image.memory(bytes, fit: BoxFit.cover)),
      const SizedBox(width: 10), SizedBox(width: 45, child: Text('${item.quantity}x', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary))),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.productName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)), Text(item.unit, style: const TextStyle(fontSize: 9, color: AppColors.textMuted))])),
      Text(_money(item.unitCost), style: const TextStyle(fontSize: 10, color: AppColors.textSecondary)), const SizedBox(width: 18), Text(_money(item.total), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
    ]));
  }
