  ...
  void _loadDraft(PurchaseRecord purchase) {
    ...
    setState(() {
      _items
        ..clear()
        ..addAll(
          purchase.items.map((record) {
            final product =
                provider.findById(record.productId) ??
                Product(
                  id: record.productId,
            ...
            final originalPresentationPrice =
                record.unitCost * presentationUnits;
            ...
            final discountedPresentationPrice = totalPaid > 0
                ...
                : (originalPresentationPrice *
                      (1 - (record.discountPercent ?? 0.0) / 100));
      ...
      children: [
        ...
        _items.isEmpty
            ? const SizedBox(
                ...
                child: Center(
                  child: Text(
                    'Selecciona productos de arriba',
                    style: TextStyle(
                      fontSize: 11,
                      color: AppColors.textSecondary,
                    ),
      ...
      children: [
        ...
        FilledButton.icon(
          ...
          label: Text(
            _saving
                ? 'Guardando...'
                : (_editing ? 'Guardar cambios' : 'Guardar compra'),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      for (final item in _items) {
        ...
        if (!_editing ||
            old == null ||
            (old.salePrice - item.salePrice).abs() > 0.0001) {
      ...
      if (costChanges.isNotEmpty) {
        ...
        updateCostIds = {
          for (final change in costChanges)
            if (decisions[change.product.id]?.updateCost == true)
              change.product.id,
        };
        if (!_editing) {
          for (final change in costChanges) {
            if (updateCostIds.contains(change.product.id)) {
              productProvider.updateProduct(
                change.product.copyWith(cost: change.newCost),
              );
            }
          }
        }
      ...
      if (_editing) {
        ...
        final updated = PurchaseRecord(
          ...
          arrivalAt: DateTime(
            ...
            _date.month,
            _date.day,
            previous.arrivalAt.hour,
            previous.arrivalAt.minute,
            previous.arrivalAt.second,
          ),
...
class _DraftPurchaseItem {
  ...
  double get unitCost => unitsPerPresentation <= 0
      ? 0.0
      : originalPresentationPrice / unitsPerPresentation;
  _DraftPurchaseItem copyWith({
    ...
    discountedPresentationPrice:
        discountedPresentationPrice ?? this.discountedPresentationPrice,
    discountPercent: discountPercent ?? this.discountPercent,
    ivaPerPresentation: clearIva
        ? null
        : (ivaPerPresentation ?? this.ivaPerPresentation),
...
class _PurchaseItemCardState extends State<_PurchaseItemCard> {
  ...
  void _step(TextEditingController controller, int delta) {
    controller.text = (_integer(controller) + delta)
        .clamp(0, 1 << 30)
        .toInt()
        .toString();
    ...
    _emit(
      recalculateDiscount: controller == _presentationController
          ? false
          : false,
    );
  }
          ...
          children: [
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: FittedBox(
                      ...
                      child: Text(
                        'Registrar ${change.newCost.toStringAsFixed(2)} como costo',
                      ),
                    ),
                  ),
                ),
            ...
            Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'No volver a preguntarme para este producto',
