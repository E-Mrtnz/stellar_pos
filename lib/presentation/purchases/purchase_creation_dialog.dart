  ...
  void _addProduct(Product product) {
    ...
    setState(() {
      ...
      if (index >= 0) {
        ...
        _items.add(_DraftPurchaseItem(product: product, purchasedQuantity: 1, bonusQuantity: 0, unitsPerPresentation: 1, originalUnitCost: null, discountedUnitCost: product.cost, discountPercent: 0, iva: null, salePrice: product.price));
      }
    });
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      for (final item in _items) {
        ...
        if (current != null && (current.cost - item.inventoryUnitCost).abs() > 0.0001) {
          final old = oldItems[item.product.id];
          final changedFromOriginal = !_editing || old == null || (old.unitCost - item.inventoryUnitCost).abs() > 0.0001;
          if (changedFromOriginal) costChanges.add(_CostChange(current, item.inventoryUnitCost));
        }
        final old = oldItems[item.product.id];
      ...
      final records = _items.map((item) {
        ...
        return PurchaseItemRecord(
          ...
          effectiveUnitCost: item.inventoryUnitCost,
        );
      }).toList();
...
class _PurchaseItemCardState extends State<_PurchaseItemCard> {
  ...
  void initState() {
    ...
    _originalController = TextEditingController(text: item.originalUnitCost?.toStringAsFixed(2) ?? '');
    _discountedController = TextEditingController(text: item.discountedUnitCost.toStringAsFixed(2));
    _discountController = TextEditingController(text: item.discountPercent > 0 ? item.discountPercent.toStringAsFixed(2) : '');
    _ivaController = TextEditingController(text: item.iva?.toStringAsFixed(2) ?? '');
  ...
  void didUpdateWidget(covariant _PurchaseItemCard oldWidget) {
    ...
    if (old.unitsPerPresentation != current.unitsPerPresentation && _integer(_presentationController) != current.unitsPerPresentation) _replace(_presentationController, '${current.unitsPerPresentation}');
    if (old.originalUnitCost != current.originalUnitCost && _numberOrNull(_originalController) != current.originalUnitCost) _replace(_originalController, current.originalUnitCost?.toStringAsFixed(2) ?? '');
  ...
  void _syncPrice({required bool fromOriginal}) {
    ...
    final source = double.tryParse((fromOriginal ? _originalController : _discountedController).text.trim().replaceAll(',', '.'));
    if (source == null || source < 0) return;
    _updating = true;
    if (discount > 0) {
      if (fromOriginal) {
        _replace(_discountedController, (source * (1 - discount / 100)).toStringAsFixed(2));
        _originalAuto = false;
        _discountedAuto = true;
      } else {
        final divisor = 1 - discount / 100;
        if (divisor > 0) {
          _replace(_originalController, (source / divisor).toStringAsFixed(2));
          _originalAuto = true;
  ...
  void _discountChanged() {
    if (_updating) return;
    final discount = _number(_discountController).clamp(0, 100).toDouble();
    final original = _numberOrNull(_originalController);
    final discounted = _numberOrNull(_discountedController);
    if (original != null) {
      _updating = true;
      _replace(_discountedController, (original * (1 - discount / 100)).toStringAsFixed(2));
      _discountedAuto = true;
      _originalAuto = false;
      _updating = false;
