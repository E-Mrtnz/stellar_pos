...
class SalesProvider extends ChangeNotifier {
  ...
  SaleRecord createSale({
    ...
    for (final entry in cartQuantities.entries) {
      ...
      if (quantity <= 0) throw StateError('La cantidad de un producto debe ser mayor que cero.');
      final lineSubtotal = product.price * quantity;
      final lineDiscount = subtotal <= 0 ? 0.0 : discountAmount * (lineSubtotal / subtotal);
    ...
    if (balanceProvider != null) {
      for (final electronicSale in electronicSales) {
        ...
        if (account.amountsForCategory(electronicSale.category).every((amount) => (amount - electronicSale.amount).abs() > 0.000001)) {
    ...
    for (final electronicSale in electronicSales) {
      ...
      final lineDiscount = subtotal <= 0 ? 0.0 : discountAmount * (lineSubtotal / subtotal);
      final providerCost = electronicSale.amount * (1 - account.commissionRate / 100);
      items.add(SaleItemRecord(
    ...
    for (final item in items.where((item) => !item.isElectronicBalance)) {
      ...
      if (product != null) productProvider.updateProduct(product.copyWith(stock: product.stock - item.quantity));
    }
    notifyListeners();
    return sale;
  }
  ...
  SaleRecord updateSale({
    ...
    for (final item in updatedItems) {
      ...
      if (item.quantity <= 0) throw StateError('La cantidad de un producto debe ser mayor que cero.');
      if (productProvider.findById(item.productId) == null) throw StateError('Uno de los productos seleccionados ya no existe.');
    ...
    if (hasElectronic) {
      for (final item in updatedItems.where((item) => item.isElectronicBalance)) {
        ...
        if (account.amountsForCategory(category).every((amount) => (amount - item.unitPrice).abs() > 0.000001)) throw StateError('Uno de los montos de recarga ya no está configurado.');
      }
    }
    ...
    for (final entry in oldPhysical.entries) {
      final product = productProvider.findById(entry.key);
      if (product != null) productProvider.updateProduct(product.copyWith(stock: product.stock + entry.value));
    }

    final subtotal = updatedItems.fold<double>(0, (sum, item) => sum + item.unitPrice * item.quantity);
    final oldDiscountRate = oldSale.subtotal <= 0 ? 0 : oldSale.discountAmount / oldSale.subtotal;
    ...
    for (final item in updatedItems) {
      ...
      final cost = product?.cost ?? (account == null ? item.cost : item.unitPrice * (1 - account.commissionRate / 100));
