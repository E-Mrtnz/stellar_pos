...
class SalesProvider extends ChangeNotifier {
  ...
  SaleRecord updateSale({
    ...
    final cardFeeRate = oldCardBase <= 0
        ? 0.0
        : oldSale.cardFeeAmount / oldCardBase;
    final newCardFeeAmount = cardBase * cardFeeRate;
    final newTotal = _totals.total(
      subtotal: newSubtotal,
      discountAmount: newDiscountAmount,
      cardFeeAmount: newCardFeeAmount,
    );
    final newDiscountPercent = oldSale.subtotal <= 0
        ? oldSale.discountPercent
        : oldDiscountRate * 100;
    final finalItems = <SaleItemRecord>[];
    for (final item in updatedItems) {
      final isElectronic = item.isElectronicBalance;
      final product = isElectronic
          ? null
          : productProvider.findById(item.productId);
  ...
  List<SaleItemRecord> availableItemsForOperation(String saleId) {
    final sale = _sales.firstWhere(
      (item) => item.id == saleId,
      orElse: () => throw StateError('La venta no existe.'),
    );
  ...
  SaleRecord returnItems({
    ...
    var updated = sale.copyWith(
      operations: [...sale.operations, operation],
      touchMetadata: true,
    );
    _sales[index] = updated;
    if (sale.items.where((item) => !item.isElectronicBalance).isNotEmpty &&
        sale.items.every((item) => !item.isElectronicBalance) &&
        availableItemsForOperation(saleId).isEmpty) {
  ...
  SaleItemRecord _withQuantity(SaleItemRecord item, int quantity) {
    ...
    return SaleItemRecord(
      ...
      quantity: quantity,
      lineSubtotal: item.lineSubtotal * ratio,
