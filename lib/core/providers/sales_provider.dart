...
class SalesProvider extends ChangeNotifier {
  ...
  SaleRecord updateSale({
    ...
    for (final item in oldSale.items) {
      if (!item.isElectronicBalance) {
        oldPhysical[item.productId] = (oldPhysical[item.productId] ?? 0) + item.quantity;
      }
    }
    ...
    if (hasElectronic) {
      ...
      for (final entry in groups.entries) {
        final registered = electronicBalanceProvider!.registerSales(
          ...
          saleId: oldSale.id,
          sales: entry.value
              .map((item) => ElectronicBalanceSale(
                    amount: item.unitPrice,
                    quantity: item.quantity,
  ...
  bool deleteSale({
    ...
    if (sale.items.any((item) => item.isElectronicBalance)) {
      if (electronicBalanceProvider == null || !electronicBalanceProvider.reverseSale(sale.id)) {
        return false;
      }
    }
