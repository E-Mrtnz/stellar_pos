...
class TicketGenerator {
  ...
  Future<List<int>> generate(SaleTicketData ticket, {bool openCashDrawer = false}) async {
    ...
    bytes.addAll(generator.feed(1));
    bytes.addAll(generator.text('GRACIAS POR SU COMPRA', styles: const PosStyles(align: PosAlign.center, bold: true, codeTable: 'CP1252')));
    bytes.addAll(generator.feed(1));
    final barcodeData = ticket.ticketNumber.codeUnits;
    if (barcodeData.isNotEmpty) bytes.addAll(generator.barcode(Barcode.code128(barcodeData), width: 2, height: 60, textPos: BarcodeText.below, align: PosAlign.center));
    if (openCashDrawer) bytes.addAll(generator.drawer(pin: PosDrawer.pin2));
    bytes.addAll(generator.feed(1)); bytes.addAll(generator.cut()); return bytes;
  }

  List<int> _summary(Generator generator, String label, double value) => generator.row([_left(label, 8), _right(_money(value), 4)]);
