...
class SaleRecord implements SyncableEntity {
  ...
  SaleRecord({required this.id, required this.ticketNumber, required this.createdAt, required this.clientId, required this.clientName, required this.paymentMethod, required List<SaleItemRecord> items, required this.subtotal, required this.discountPercent, required this.discountAmount, required this.cardFeeAmount, required this.total, required this.received, required this.change, SyncMetadata? metadata}) : items = List.unmodifiable(items), metadata = metadata ?? SyncMetadata(createdAt: createdAt.toUtc(), updatedAt: createdAt.toUtc());

  SaleRecord copyWith({String? id, String? ticketNumber, DateTime? createdAt, String? clientId, String? clientName, String? paymentMethod, List<SaleItemRecord>? items, double? subtotal, double? discountPercent, double? discountAmount, double? cardFeeAmount, double? total, double? received, double? change, SyncMetadata? metadata, bool touchMetadata = false}) => SaleRecord(id: id ?? this.id, ticketNumber: ticketNumber ?? this.ticketNumber, createdAt: createdAt ?? this.createdAt, clientId: clientId ?? this.clientId, clientName: clientName ?? this.clientName, paymentMethod: paymentMethod ?? this.paymentMethod, items: items ?? this.items, subtotal: subtotal ?? this.subtotal, discountPercent: discountPercent ?? this.discountPercent, discountAmount: discountAmount ?? this.discountAmount, cardFeeAmount: cardFeeAmount ?? this.cardFeeAmount, total: total ?? this.total, received: received ?? this.received, change: change ?? this.change, metadata: metadata ?? (touchMetadata ? this.metadata.touch() : this.metadata));

  Map<String, dynamic> toMap() => {'id': id, 'ticketNumber': ticketNumber, 'createdAt': createdAt.toIso8601String(), 'clientId': clientId, 'clientName': clientName, 'paymentMethod': paymentMethod, 'items': items.map((item) => item.toMap()).toList(), 'subtotal': subtotal, 'discountPercent': discountPercent, 'discountAmount': discountAmount, 'cardFeeAmount': cardFeeAmount, 'total': total, 'received': received, 'change': change, 'metadata': metadata.toMap()};

  factory SaleRecord.fromMap(Map<String, dynamic> map) => SaleRecord(id: map['id']?.toString() ?? IdGenerator.newId(), ticketNumber: map['ticketNumber']?.toString() ?? '', createdAt: _date(map['createdAt']), clientId: map['clientId']?.toString(), clientName: map['clientName']?.toString() ?? '', paymentMethod: map['paymentMethod']?.toString() ?? '', items: _items(map['items']), subtotal: _double(map['subtotal']), discountPercent: _double(map['discountPercent']), discountAmount: _double(map['discountAmount']), cardFeeAmount: _double(map['cardFeeAmount']), total: _double(map['total']), received: _double(map['received']), change: _double(map['change']), metadata: _metadata(map['metadata']));
