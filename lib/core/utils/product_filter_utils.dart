...
class ProductFilterUtils {
  ...
  static List<Map<String, dynamic>> apply({
    ...
    return filtered
        .where((product) => ProductUtils.asString(product['category']) == selectedTag)
        .toList();
  }
  ...
  static List<Map<String, dynamic>> _filterByStatus(
    ...
    if (selectedFilter.startsWith('brand:')) {
      ...
      return products
          .where((product) =>
              ProductUtils.asString(product['brand']).toLowerCase() == brand)
          .toList();
    }
    if (selectedFilter.startsWith('distributor:')) {
      final distributor = selectedFilter.substring(12).trim().toLowerCase();
      return products
          .where((product) =>
              ProductUtils.asString(product['department']).trim().toLowerCase() == distributor)
          .toList();
    }
